...
| Роль          | Что делает                                                                                                     |
|---------------|----------------------------------------------------------------------------------------------------------------|
| **Админ**     | Поднимает сервер (`Сервер/local` или `Сервер/cloud`), правит один файл `Клиент/AI_Suggester/ai_macro/Settings.xba` с адресом, пересобирает `.oxt`, рассылает сотрудникам по почте. |
