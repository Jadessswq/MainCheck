...
**Qwen2.5-32B-Instruct** — на нашем боевом бенчмарке
(текст со 4 классическими ошибками управления: «согласно распоряжения»,
«в … округе», «благодаря решения», «во исполнения») показала:
- **155 с** на холодном старте, ~15–30 с после прогрева
- **все 4 ошибки** найдены и исправлены корректно
- ответ сразу в нужном формате (без рассуждений)

Для сравнения qwen3:30b-a3b (MoE) на той же задаче — 277 с и в ответе
рассуждения вместо правок. Несмотря на номинально более высокую скорость
MoE-варианта, он включает thinking-режим, который игнорирует `think:false`
на некоторых весах.

| Модель                | RAM (Q4) | CPU-время на типовой текст¹ | Качество (рус.офиц.) | Команда                        |
|-----------------------|----------|------------------------------|----------------------|--------------------------------|
| **qwen2.5:32b** ⭐    | ~19 ГБ   | **15–30 с**                  | **Эталон**           | `ollama pull qwen2.5:32b`      |
| qwen2.5:14b           | ~9 ГБ    | 8–15 с                       | Хорошее              | `ollama pull qwen2.5:14b`      |
| gemma3:27b            | ~17 ГБ   | 20–40 с                      | Хорошее              | `ollama pull gemma3:27b`       |
| mistral-small3.2:24b  | ~14 ГБ   | 12–25 с                      | Хорошее              | `ollama pull mistral-small3.2:24b` |
| ~~qwen3:30b-a3b~~     | ~18 ГБ   | 180–280 с                    | Болтлива             | НЕ рекомендуется               |

¹ После прогрева, на 28 потоках CPU, типовой фрагмент 200–500 слов.

> **Если RAM <20 ГБ** — `qwen2.5:14b` достаточно для большинства типовых ошибок,
> поэтому смело берите её на машинах послабее.
...
Модель занимает ≈ 19 ГБ на диске. Чтобы освободить место:
...
- 16 ГБ → `qwen2.5:14b` (9 ГБ) или `qwen2.5:7b`
